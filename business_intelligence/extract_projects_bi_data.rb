# extract_projects_bi_data.rb - Extract projects' BI data
#
...
out_path =
  if ARGV.size == 1
    ...
    "projects_bi_dump_#{Time.now.strftime("%Y%m%d_%H%M%S")}.csv"
  end
...
CSV.open(out_path, "wb") do |csv|
  ...
  Project.includes(:team).find_each do |project|
    values_by_id = {}

    project.custom_field_values.each do |cfv|
